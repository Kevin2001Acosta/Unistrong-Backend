...
describe("RoutineService", () => {
    ...
    describe("createRoutine", () => {
        ...
        it("Debe crear una rutina correctamente si el coach existe", async () => {
            ...
            Coach.findByPk.mockResolvedValue(mockCoach);
            Routines.create.mockResolvedValue(mockRoutine);
        ...
        it("Debe lanzar un error si el coach no existe", async () => {
            ...
            await expect(RoutineService.createRoutine(mockRoutineData)).rejects.toThrow("Coach no encontrado");
            expect(Coach.findByPk).toHaveBeenCalledWith(1);
    ...
    describe("getAllRoutines", () => {
        it("Debe retornar todas las rutinas si existen", async () => {
            ...
            Routines.findAll.mockResolvedValue(mockRoutines);
        ...
        it("Debe lanzar un error si ocurre un problema al obtener las rutinas", async () => {
            ...
            Routines.findAll.mockRejectedValue(new Error(errorMessage));
            await expect(RoutineService.getAllRoutines()).rejects.toThrow(`Error al obtener las rutinas: ${errorMessage}`);
        });
    });
    describe("assignRoutineToClient", () => {
        const mockAssignRoutineData = {
            clientId: 1,
            routineId: 1,
        ...
        it("Debe asignar una rutina a un cliente si ambos existen", async () => {
            ...
            Routines.findByPk.mockResolvedValue({ id: 1 });
            ClientRoutines.create.mockResolvedValue({});
        ...
        it("Debe lanzar un error si el cliente no existe", async () => {
            ...
            await expect(RoutineService.assignRoutineToClient(mockAssignRoutineData)).rejects.toThrow("El cliente especificado no existe.");
            expect(Client.findByPk).toHaveBeenCalledWith(1);
        ...
        it("Debe lanzar un error si la rutina no existe", async () => {
            Client.findByPk.mockResolvedValue({ id: 1 });
            Routines.findByPk.mockResolvedValue(null);
            await expect(RoutineService.assignRoutineToClient(mockAssignRoutineData)).rejects.toThrow("La rutina especificada no existe.");
            expect(Client.findByPk).toHaveBeenCalledWith(1);
            expect(Routines.findByPk).toHaveBeenCalledWith(1);
            expect(ClientRoutines.create).not.toHaveBeenCalled();
        });
    ...
    describe("assignRoutineByEmail", () => {
        ...
        it("Debe asignar una rutina al cliente usando su correo electrónico", async () => {
            ...
            Routines.findByPk.mockResolvedValue(mockRoutine);
            ClientRoutines.create.mockResolvedValue({});
            // Ejecutar el servicio
            await RoutineService.assignRoutineByEmail(mockUser.email, mockRoutine.id, new Date());
        ...
        it("Debe lanzar un error si el usuario no existe", async () => {
            ...
            await expect(RoutineService.assignRoutineByEmail("test@example.com", 1, new Date())).rejects.toThrow("Usuario no encontrado.");
        });
        it("Debe lanzar un error si la rutina no existe", async () => {
            Users.findOne.mockResolvedValue(mockUser);
            Client.findOne.mockResolvedValue(mockClient);
            Routines.findByPk.mockResolvedValue(null);
            await expect(RoutineService.assignRoutineByEmail(mockUser.email, 999, new Date())).rejects.toThrow("La rutina especificada no existe.");
        });
    });
});
