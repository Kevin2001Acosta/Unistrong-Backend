...
describe("VerificationService", () => {
    ...
    describe("createVerificationCode", () => {
        it("debería crear un código de verificación nuevo y actualizar los anteriores a verificados", async () => {
            ...
            Verification.update.mockResolvedValue([1]);
            Verification.create.mockResolvedValue(true);
            await VerificationService.createVerificationCode(userId, code, type);
            expect(Verification.update).toHaveBeenCalledWith({ verified: true }, { where: { userId, active: true, verified: false } });
    ...
    describe("verifyCodeoOfPassword", () => {
        it("debería lanzar error si el email no está registrado", async () => {
            ...
            await expect(VerificationService.verifyCodeoOfPassword(email, "123456")).rejects.toThrow("El email no está registrado");
        });
        it("debería lanzar error si el código de verificación es inválido", async () => {
            const email = "test@example.com";
            const code = "invalid";
            userServices.getUserByEmail.mockResolvedValue({ id: 1 });
            Verification.findOne.mockResolvedValue(null);
            await expect(VerificationService.verifyCodeoOfPassword(email, code)).rejects.toThrow("Código de verificación inválido");
        });
        it("debería lanzar error si el código de verificación está expirado", async () => {
            ...
            userServices.getUserByEmail.mockResolvedValue({ id: 1 });
            Verification.findOne.mockResolvedValue(expiredVerification);
            await expect(VerificationService.verifyCodeoOfPassword(email, code)).rejects.toThrow("Código de verificación expirado");
        });
        it("debería verificar el código correctamente si es válido", async () => {
            ...
            userServices.getUserByEmail.mockResolvedValue({ id: 1 });
            Verification.findOne.mockResolvedValue(validVerification);
            const result = await VerificationService.verifyCodeoOfPassword(email, code);
    ...
    describe("verifyCodeoOfEmail", () => {
        it("debería lanzar error si el usuario no está registrado", async () => {
            const userId = 999;
            userServices.getUserById.mockResolvedValue(null);
            await expect(VerificationService.verifyCodeoOfEmail(userId, "123456")).rejects.toThrow("El usuario no está registrado");
        });
        it("debería lanzar error si el código de verificación es inválido", async () => {
            const userId = 1;
            const code = "invalid";
            userServices.getUserById.mockResolvedValue({ id: 1 });
            Verification.findOne.mockResolvedValue(null);
            await expect(VerificationService.verifyCodeoOfEmail(userId, code)).rejects.toThrow("Código de verificación inválido");
        });
        it("debería lanzar error si el código de verificación está expirado", async () => {
            const userId = 1;
            const code = "expired";
            ...
            userServices.getUserById.mockResolvedValue({ id: 1 });
            Verification.findOne.mockResolvedValue(expiredVerification);
            await expect(VerificationService.verifyCodeoOfEmail(userId, code)).rejects.toThrow("Código de verificación expirado");
        });
        it("debería verificar el código correctamente si es válido", async () => {
            const userId = 1;
            const code = "123456";
