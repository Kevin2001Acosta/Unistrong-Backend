...
class AdminService {
    ...
    async createUserAnyType(userData) {
        try {
    ...
    async assignCoachToClient(clientEmail, coachEmail) {
        ...
        if (!client) {
            throw createError(404, "El cliente no tiene un perfil en la tabla Clients.");
        }
        if (client.coach_id) {
        ...
        if (!coach) {
            throw createError(404, "El coach no tiene un perfil en la tabla Coaches.");
        }
        //Asignar el coach al cliente
        client.coach_id = coach.id;
        await client.save();
    }
    async assignNutritionistToClient(clientEmail, nutriEmail) {
        //Buscar al cliente por email
        const clientUser = await Users.findOne({
            where: { email: clientEmail, userType: UserType.CLIENT },
        });
